...
class VectorStore:
    ...
    def add_image(self, debate_id: int, image_path: str, vector: np.ndarray, ocr_text: str) -> int:
    ...
    def process_and_add_image(self, debate_id: int, image_path: str) -> int:
        ...
        if debate_id <= 0:
            print(f"WARNING: Invalid debate_id: {debate_id}, image may not be associated correctly")
        else:
            ...
            if not debate:
                ...
                if latest_debate:
                    ...
                    print(f"Using latest debate ID {new_debate_id} instead of {debate_id}")
        ...
        if not os.path.exists(processing_path):
            print(f"WARNING: Image file not found at {processing_path}, trying alternative paths")
            ...
            for path in alt_paths:
                ...
                raise FileNotFoundError(f"Image file not found at any expected location. Tried: {alt_paths}")
        ...
        try:
            ...
            print(f"Image processing successful. Description: {image_data.description[:100] if hasattr(image_data, 'description') else 'No description'}...")
            ...
            image_id = self.add_image(
                ...
                ocr_text=", ".join(image_data.ocr) if hasattr(image_data, 'ocr') else ""
            )
            ...
            print(f"Failed to process image with Mistral API, but saved basic record with image_id={image_id}, debate_id={debate_id}")
        ...
        if image_id:
            ...
            if result and result[0] != debate_id:
                print(f"WARNING: Image {image_id} is associated with debate {result[0]}, not {debate_id}")
                self.cursor.execute("UPDATE image SET debate_id = ? WHERE id = ?", (debate_id, image_id))
                self.conn.commit()
                print(f"Fixed: Image {image_id} is now associated with debate {debate_id}")
        
        return image_id or 0  # Ensure we always return an integer
    
    def search(self, query_vector: np.ndarray, k: int = 5) -> List[Tuple[str, float, str, str]]:
    ...
    def search_by_text(self, query_text: str, k: int = 5) -> List[Tuple[str, float, str, str]]:
        ...
        try:
            ...
            query_embedding = self.processer.process_instruction(query_text).instruction_feats
    ...
    def _text_based_search_fallback(self, query_text: str, k: int = 5) -> List[Tuple[str, float, str, str]]:
        ...
        for img_id, img_path, ocr, debate_id, tldr, summary in self.cursor.fetchall():
            ...
            if matching_words > 0:
                ...
                if text_words > 0:
                    ...
                    score = matching_words / (len(query_words) + text_words - matching_words)
                else:
                    score = 0.0
