...
async function performAPISearch(query) {
    try {
        ...
        const url = `/api/search-debates?query=${encodeURIComponent(query)}&minimum_score=${minimum_score}&include_all=${include_all}`;
        ...
        const relevantResults = data.debates.filter(debate => debate.score > 0).length;
        ...
        if (data.debates.length === 0) {
            ...
            header.innerHTML = `
                ...
                <p>${data.debates.length} debates found${hasScores ? `, ${relevantResults} with matches` : ''}</p>
...
function displayDebates(debates, isSearchResult = false, hasScores = false) {
    ...
    if (debates.length === 0) {
        container.innerHTML = `
            <div class="no-debates">
                <p>${isSearchResult ? 'No matches found' : 'No whiteboard images found'}.</p>
                <p>${isSearchResult ? '<a href="/">Back to all debates</a>' : 'Click <a href="/record">here</a> to add a new one'}.</p>
            </div>
        `;
        return;
    }
    ...
    debates.forEach(debate => {
        ...
        console.log(`Processing debate ID ${debate.id} with image path: "${debate.image_path}"${isSearchResult ? ` (score: ${debate.score.toFixed(4)})` : ''}`);
        ...
        if (debate.image_path) {
            ...
            console.log(`Generated image paths for debate ${debate.id}:`, imagePathsToTry);
        ...
        imgElement.onerror = function() {
            console.warn(`Failed to load image from ${this.src} for debate ${debate.id}`);
            
            if (imagePathsToTry.length > 0 && currentPathIndex < imagePathsToTry.length - 1) {
                // Try next path
                currentPathIndex++;
                ...
                console.error(`All image paths failed for debate ${debate.id}, using placeholder`);
        ...
        imgElement.onload = function() {
            console.log(`✅ Successfully loaded image for debate ${debate.id} from: ${this.src}`);
        };
        ...
        if (debate.score > 0) {
            ...
            scoreHtml = `<span class="score-badge ${scorePercent > 70 ? 'high' : scorePercent > 40 ? 'medium' : 'low'}">${scorePercent}%</span>`;
        ...
        detailsDiv.innerHTML = `
            ...
            <p class="debate-summary">${debate.summary?.substring(0, 100) || ""}${debate.summary?.length > 100 ? '...' : ''}</p>
